...
def log_upgrade_check():
    ...
    try:
        ...
        if version_gt(latest_release, __version__):
            click.echo(f"There's a new version of cellxlineage available ({latest_release})!", err=True)
            click.echo("To upgrade, run the following: pip install --upgrade cellxlineage\n", err=True)
